...
public class RezultatArrayAdapter extends ArrayAdapter<Rezultat>{
LayoutInflater mInflater;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		Rezultat tmp = getItem(position);
		ViewHolder holder;
		// When convertView is not null, we can reuse it directly, there is no need
		// to reinflate it. We only inflate a new View when the convertView supplied
		// by ListView is null.
		if (convertView == null) {
			convertView = mInflater.inflate(R.layout.rezultat_layout, null);
		...
		holder.one.setText(tmp.getName()); //Step 4.8 POPRAVI
		holder.two.setText(""+tmp.getStevilo_poskusov()); //Step 4.8 POPRAVI
